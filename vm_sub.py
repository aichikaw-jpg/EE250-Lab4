...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe("aichikaw/ipinfo")
    
    #Add the custom callbacks by indicating the topic and the name of the callback handle
    client.message_callback_add("aichikaw/ipinfo", on_message_from_ipinfo)

    print("topic: datetime")
    client.subscribe("aichikaw/datetime")
    client.message_callback_add("aichikaw/datetime", on_message_from_datetime)

    print("topic: date")
    client.subscribe("aichikaw/currentDate")
    client.message_callback_add("aichikaw/currentDate", on_message_from_currentDate)

    print("topic: date")
    client.subscribe("aichikaw/currentTime")
    client.message_callback_add("aichikaw/currentTime", on_message_from_currentTime)
...
#custom message callback for datetime
def on_message_from_datetime(client,userdata, message):
    print("Custom callback - Date Time: " + message.payload.decode())

#custom message callback for current date
def on_message_from_currentDate(client, userdata, message):
    print("Custom callback - current date: " + message.payload.decode())

#custom message callback for current time
def on_message_from_currentTime(client, userdata, message):
